...
async function main() {
  ...
  for (let i = 0; i < tvURLs.length; i++) {
    ...
    const title = await nightmare
      ...
      .evaluate(() => {
        data = []
        if (document.querySelectorAll("div.tr:nth-child(2) > div:nth-child(2)").length>0){
        lang = document.querySelector("div.tr:nth-child(2) > div:nth-child(2)").innerText;}
        else{
            lang =""
        }
        if(document.querySelectorAll("div.tr:nth-child(3) > div:nth-child(2)").length>0){
            director = document.querySelector("div.tr:nth-child(3) > div:nth-child(2)").innerText;
        }else{
            director = ""
        }
        if(document.querySelectorAll("div.tr:nth-child(4) > div:nth-child(2)").length>0){
            stars = document.querySelector("div.tr:nth-child(4) > div:nth-child(2)").innerText;
        }else{
            stars = ""
        }
