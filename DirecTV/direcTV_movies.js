...
movieDB = []
for (i = 0; i < pages; i++) {
    apiURLS[i] = 'https://www.directv.com/json/movies/search?CAROUSELS_FILTER=movies&Watch=all&hideAdultContent=true&orderby=ReleaseDate:DESCENDING&resultsetend=' + pageSize * (i + 1) + '&resultsetstart=' + (i * pageSize + 1)
}
console.log(apiURLS);
(async () => {
    ...
    /* for (i = 0; i < apiURLS.length; i++) {
        ...
        var movieDetails = await page.evaluate(() => {
            ...
            for (j = 0; j < data.titles.length; j++) {
                jsonData.push({
                    movieID: data.titles[j].tmsProgramID,
                    movieLink: 'https://www.directv.com' + data.titles[j].detailsLinkUrl,
                    movieTitle: data.titles[j].title,
    ...
    fs.writeFileSync('movie_id.json', JSON.stringify(movieDB, null, 2)) */
