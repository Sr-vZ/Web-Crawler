...
var data = JSON.parse(fs.readFileSync('youtube_tvf_dump.json', 'utf-8'))
    ...
    return accumulator.then(function (results) {
        return nightmare.goto(url)
            ...
            .catch(error => {
                ...
                fs.appendFileSync('yt_tvf_errors.json', JSON.stringify(errorData))
            });
    });
}, Promise.resolve([])).then(function (results) {
    fs.appendFileSync("yt_tvf_details_dump.json", JSON.stringify(alldata));
    console.log(alldata);
    nightmare.end();
});