...
temp = JSON.parse(fs.readFileSync('crackle_movies.json'))

// https://web-api-us.crackle.com/Service.svc/nowplaying/2505101/3566/US?format=json

for (i = 0; i < temp.length; i++) {
    movieURLS[i] = 'https://web-api-us.crackle.com/Service.svc/nowplaying/' + temp[i].mediaID + '/' + temp[i].movieID + '/US?format=json'
}
...
(async () => {
    ...
    for (p = 0; p < movieURLS.length; p++) {
        ...
        var mediaDetails = await page.evaluate((title, imgLink, synopsis) => {
            ...
            d = new Date( data.Result.MediaTooltips[0].ReleaseDate)
            ...
            duration = data.Result.MediaTooltips[0].DurationInSeconds

            t = data.Result.MediaTooltips[0].Cast
            stars = []
            ...
            link = 'https://www.sonycrackle.com/' + title.toLocaleLowerCase().replace(/ /g,'-').replace(':','')
            jsonData.push({
                ...
                directors: data.Result.MediaTooltips[0].Directors //Array of string | If multiple director names available.
    ...
    fs.writeFileSync('crackle_movies_cast.json', JSON.stringify(kanopyDB, null, 2))
