      ...
      for (j = 0; j < fetchTimes; j++) {
        ...
        var episodeDetails = await page.evaluate((seriesLink, language, seasonTitle) => {
          ...
          for (k = 0; k < data.episodes.length; k++) {
            ...
            imageLink  = 'https://akamaividz.zee5.com/resources/' + data.episodes[k].id+'/list/270x152/'+data.episodes[k].list_image
            jsonData.push({
              ...
              episode_number: parseInt(data.episodes[k].index), //Integer | For episode number
              ...
              link: seriesLink +'/'+ data.episodes[k].title.toLocaleLowerCase().replace(/ /g, '-').replace(/-+/g, '-').replace("'", "").replace(",", "") + '/' + data.episodes[k].id, // String | Episode link
              ...
              image_link: imageLink, //String | episode image link.
...
// https://akamaividz.zee5.com/resources/0-1-kumbhkaran-season1_287989183-episode_1550880411/list/270x152/kumbhkaran_ep1_740.jpg
            })
          }
          return jsonData
